...
//* Supabase
import { insertCarLoanDraftApplication } from '@/lib/supabase/draft-loan-application/car-loan-update'
import { getActiveOriginatorByCode } from '@/lib/supabase/originators/select'
import {
...
// Car-loan variant — accepts appOriginator
import { prepareJointApplicationJson } from '@/lib/occ/car-loan/prepareJointApplicationJson'
...
export async function POST(request: Request) {
  ...
  // ─── Originator validation (car-loan only) ─────────────────────────────
  // Strict by design: this is a new endpoint, no transition concern.
  if (!appOriginator || typeof appOriginator !== 'string') {
    return NextResponse.json(
      {
        success: false,
        error: 'Missing or invalid appOriginator in request body.',
      },
      { status: 400 },
    )
  }

  const originatorRow = await getActiveOriginatorByCode(appOriginator)
  if (!originatorRow) {
    return NextResponse.json(
      {
        success: false,
        error: `appOriginator '${appOriginator}' is not a known active originator.`,
      },
      { status: 400 },
    )
  }
  ...
  await insertCarLoanDraftApplication({
    ...draftApplicationInsertData,
    app_originator: appOriginator,
  })

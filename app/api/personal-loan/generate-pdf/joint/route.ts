...
import { enrichJointLoanData } from '@/lib/pdf/enrichJointLoanData'
import { generateJointLoanPDF } from '@/lib/pdf/generateJointLoanPDF'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const enrichedData = await enrichJointLoanData(parsedJsonRequest, dbFields)

    // Generate PDF
    const pdfBuffer = await generateJointLoanPDF(enrichedData)
    ...
    return new NextResponse(pdfBuffer as unknown as BodyInit, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="joint-loan-application-${loan_application_number}.pdf"`,
        'Content-Length': pdfBuffer.length.toString(),
      },
    })
  } catch (error) {
    console.error('Error in joint PDF generation route:', error)
    return NextResponse.json(
      {
        success: false,
        error: 'An error occurred while generating the joint PDF.',

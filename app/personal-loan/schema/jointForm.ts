...
export const employmentsToExclude = ['BNF', 'UEM', 'RTD', 'STP', 'STU']
export const employmentsToExcludeForIncome = ['UEM', 'RTD', 'STP', 'STU']
    ...
    mobileNumber: z
      ...
      .refine(
        (val) => {
          ...
          return phoneRegExp.test(val) && val.length <= 12
        },
        {
  ...
  .superRefine(
    (
      ...
      if (
        ...
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Net income must be more than $0',
          path: ['netIncome'],
        })
      }
    }
...
const productsSchema = z.object({
  transactionalAccounts: z.array(z.string()).optional(),
  savingAccounts: z.array(z.string()).optional(),
...
const generalAndProducts = z.intersection(generalFormSchema, productsSchema)
const PrelimsAndServices = z.intersection(generalAndProducts, servicesSchema)
const ServicesAndFinalQns = z.intersection(
  PrelimsAndServices,
  finalQuestionsChema
)

export const jointMembershipFormSchema = z.intersection(
  ServicesAndFinalQns,

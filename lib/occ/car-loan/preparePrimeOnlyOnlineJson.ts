...
type primeOnlineJsonProps = {
  // Car-loan addition: dealership originator code (PK from api.tblOriginators).
  // Used as both clientNumber and value of the originator object below.
...
export async function preparePrimeOnlineJson({
  ...
  // Car-loan: originator is sourced from the per-application appOriginator value
  // (validated by the route against api.tblOriginators) rather than hardcoded.
  const originator = {
    clientNumber: appOriginator,
    value: appOriginator,
  }

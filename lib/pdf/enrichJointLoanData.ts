...
export async function enrichJointLoanData(
  ...
  // Find prime employment type description
  const primeEmploymentTypeDescription = employmentTypesFallback.find(
    (empType) => empType.type === jsonRequest.primeEmployment.employmentType
  )?.desc

  // Find prime occupation description
  const primeOccupationDescription = occupationFallback.find(
    (occ) => occ.activity_code === jsonRequest.primeEmployment.occupation
  )?.activity_name

  // Find joint employment type description
  const jointEmploymentTypeDescription = employmentTypesFallback.find(
    (empType) => empType.type === jsonRequest.jointEmployment.employmentType
  )?.desc

  // Find joint occupation description
  const jointOccupationDescription = occupationFallback.find(
    (occ) => occ.activity_code === jsonRequest.jointEmployment.occupation
  )?.activity_name
  ...
  const enrichedData: EnrichedJointLoanData = {
    // Add database fields
    ...
    jointPreliminaryQuestions: jsonRequest.jointPreliminaryQuestions as {
      isNzCitizen: string
      citizenship: string
      isNzResident: string
    ...
    jointPersonalDetails: jsonRequest.jointPersonalDetails as {
      title: string
      firstName: string
      middleName: string
      lastName: string
    ...
    jointContactDetails: jsonRequest.jointContactDetails as {
      residentialAddress: string
      residentialAddressPxid: string
      accommodationType: string
      residentialEffectiveMonth: string
    ...
    jointDriversLicence: jsonRequest.jointDriversLicence as {
      licenceType: string
      licenceNumber: string
      licenceVersion: string
      licenceIssueDate: string
    ...
    jointPassport: jsonRequest.jointPassport as { passportNumber: string },
    jointFirearmsLicence: jsonRequest.jointFirearmsLicence as {
      firearmsNumber: string
    },
    jointBirthCertificate: jsonRequest.jointBirthCertificate as {
      birthCertificateRegNo: string
    },
    jointKiwiAccessCard: jsonRequest.jointKiwiAccessCard as {
      kiwiAccessCardNumber: string
    },
    jointCommunityServiceCard: jsonRequest.jointCommunityServiceCard as {
      communityServiceCardNumber: string
    },
    jointgoldCard: jsonRequest.jointgoldCard as { goldCardNumber: string },
    jointstudentID: jsonRequest.jointstudentID as {
      currentStudentCardNumber: string
    },
    ...
    primeEmployment: {
      ...
      employmentTypeDescription: primeEmploymentTypeDescription,
      occupationDescription: primeOccupationDescription,
    },
    jointEmployment: {
      ...(jsonRequest.jointEmployment as unknown as {
        employmentType: string
        schoolorTertiaryInst: string
        occupation: string
        employerName: string
      ...
      employmentTypeDescription: jointEmploymentTypeDescription,
      occupationDescription: jointOccupationDescription,
    },
    formFinancialDetails: {
